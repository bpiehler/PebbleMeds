...
- [ ] Extract `getFixedTimes`, `getIntervalTimes`, `getNextDoseTimes` to `src/pkjs/schedule.js` (no Pebble deps, CommonJS exports)
- [ ] Update `index.js` to `require('./schedule')`
- [ ] Add Jest as dev dependency; add `"test": "jest"` script to `package.json`
- [ ] Write `tests/schedule.test.js` — ~25 cases covering:
  - Fixed: next dose today, next dose tomorrow (all today's past), multiple times/day, 48h window
  - Interval: with `lastTakenTs`, without (first run), 48h lookahead
  - Edge cases: dose exactly at `fromTs`, interval spanning midnight

### Phase 4 — Timeline API, App Icon & Round 2 Polish
- [ ] Fix Timeline bugs (done in commit): wrong API endpoint (`getpebble.com` → `rebble.io`), spurious `timelineSubscribe` calls removed, icon changed to `NOTIFICATION_REMINDER`
- [ ] Extract Timeline logic from `index.js` to `src/pkjs/timeline.js`; `index.js` requires it
- [ ] Pin metadata: confirm privacy-mode title/body behaviour; verify `openWatchApp` + `launchCode` (confirmed correct)
- [ ] App icon: 25×25 PNG in `resources/images/`; register with `"type": "bitmap", "menuIcon": true` in `appinfo.json` resources
- [ ] Round 2 / gabbro layout refinements (260×260 canvas, larger pill `PILL_R`, better text sizing)
- [ ] gabbro touchscreen: **no swipe API in C SDK** — firmware maps touch→buttons automatically; no code change needed; verify UX with physical device
- [ ] Adherence log: `persist_write_data` on C side to complement the existing `localStorage` log in JS
